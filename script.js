...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── 15. GALERÍA — scroll horizontal cinematic ── */
  (function initGaleriaHScroll() {
    const outer = document.getElementById('galeriaOuter');
    ...
    function setOuterHeight() {
      if (!mq.matches) { outer.style.height = ''; return; }
      const overflow = Math.max(0, track.scrollWidth - window.innerWidth);
      outer.style.height = (overflow + window.innerHeight) + 'px';
    }
    ...
    setOuterHeight();
    window.addEventListener('resize', () => { setOuterHeight(); onHScroll(); }, { passive: true });
